...
def _effective_floor(pd, d, l):
    """Настроенный порог (l) — это "типичный" последний PD перед слётом,
    подобранный по НОРМАЛЬНОЙ последовательности (например для нестрахованных
    домов с d=2: 7→5→3→слёт, порог=3). Но если у конкретного дома PD идёт
    по другой чётности (8→6→4→2→слёт), последовательность физически никогда
    не попадёт ровно на 3 — она перескочит через него и реально упрётся в 2.
    Раньше порог был жёстко зафиксирован (например 3 для нестрахованных), и
    для таких "чётных" домов PD клэмпился на 3 (никогда не показывая 2), а
    время слёта считалось так, будто порог всё-таки 3 — из-за чего слёт
    показывался на один шаг РАНЬШЕ, чем по факту падает дом.

    Здесь просто подгоняем порог под чётность конкретного PD, чтобы
    последовательность "pd, pd-d, pd-2d, ..." гарантированно попадала на
    него точно (для нестрахованных домов на 3 или на 2 — смотря что реально
    достижимо от данного стартового PD)."""
    if d <= 0:
        d = 1
    return l - ((pd - l) % d)
...
def calc_expiry_from_pdl(pd, d, l, now=None):
    ...
    if d <= 0:
        d = 1
    eff_l = _effective_floor(pd, d, l)
    # +1 час намеренно: дом реально падает не в момент, когда PD впервые
    # ПОКАЗЫВАЕТ порог, а на следующей часовой границе после этого — грубо
    # говоря, порог — это ещё "последний живой" PD, а не сам момент слёта.
    hours_left = max((pd - eff_l) // d + 1, 0)
    return _add_valid_hours(now, hours_left)
